    ...
    return (
      <div className="flex flex-col gap-8 p-8 max-w-4xl mx-auto w-full">
        ...
        <div className="grid grid-cols-3 gap-4">
          <StatsCard icon={TrendingUp} label="Jami tahlillar" value="4" color="text-green-600" bg="bg-green-50" />
          <StatsCard icon={Sprout} label="Faol dalalar" value="3" color="text-blue-600" bg="bg-blue-50" />
          <StatsCard icon={CloudRain} label="Sug'orish kerak" value="1" color="text-amber-600" bg="bg-amber-50" />
        </div>
            ...
            {MOCK_HISTORY.slice(0, 2).map((item) => {
              ...
              return (
                <div key={item.id} className="bg-white rounded-2xl p-5 border border-border shadow-sm flex items-center gap-4">
                  ...
                  <span className={`text-xs font-bold px-2.5 py-1 rounded-full text-white flex-shrink-0 ${rc.badge}`}>
                    {rc.label}
                  </span>
                </div>
              );
